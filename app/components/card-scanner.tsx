...
function ScanResult({ result, owned, onAdd, onRetry }: { result: ResolvedCardScan; owned: boolean; onAdd: (impression: CollectionImpression) => void; onRetry: () => Promise<void> }) {
  ...
  return <div className="collection-scanner-result"><p className="eyebrow">Carte détectée · confiance {confidence === "high" ? "élevée" : "moyenne"}</p><div className="collection-scanner-card"><span className="collection-scanner-art">{impression.variant.imageUrl ? <CachedCardImage src={impression.variant.imageUrl} alt={`Carte ${impression.row.name}`} /> : <span aria-hidden="true">◇</span>}</span><div><h3>{impression.row.name}</h3><p>{impression.setName} · #{impression.variant.number}</p><p className={`collection-scanner-ownership ${owned ? "is-owned" : "is-missing"}`}>{owned ? "✓ Déjà dans ta collection" : "○ Carte manquante de ta collection"}</p></div></div><div>{!owned ? <button type="button" onClick={() => onAdd(impression)}>Ajouter à ma collection</button> : null}<button type="button" className="secondary" onClick={() => { void onRetry(); }}>Scanner à nouveau</button></div></div>;
}
