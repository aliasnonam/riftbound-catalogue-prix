...
function CollectionTools({ impressions, isMobileApp }: { impressions: CollectionImpression[]; isMobileApp: boolean }) {
  ...
  return <main className="collection-shell collection-tools-shell">
    ...
    <div className="collection-tools-stack">
      ...
      {isMobileApp ? <><PurchaseMode impressions={impressions} /><section className="purchase-sessions-entry collection-tools-action-card"><div><p className="eyebrow">{en ? "Purchase mode" : "Mode achat"}</p><h2>{en ? "Potential purchases" : "Achats potentiels"}</h2><p>{en ? "Review seller sessions, compare totals and add the cards you actually bought to your collection." : "Retrouve tes sessions chez les vendeurs, compare les totaux et ajoute à ta collection les cartes réellement achetées."}</p></div><Link href="/outils/achats">{en ? "View purchases" : "Voir les achats"}</Link></section></> : null}
