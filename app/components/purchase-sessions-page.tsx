...
function PurchaseSessionDetail({ session, impressions, onBack }: { session: PurchaseSession; impressions: CollectionImpression[]; onBack: () => void }) {
  ...
  const finishSession = () => {
    for (const item of session.items) {
      ...
      if (impression) collection.addPurchasedCopy(impression, hasPurchaseFinish(item) && item.finish === "foil");
    }
  ...
  return <section className="purchase-session-detail"><button type="button" className="purchase-back-button" onClick={onBack}>← {en ? "All sessions" : "Toutes les sessions"}</button><div className="purchase-session-detail-heading"><div><p className="eyebrow">{en ? "Potential purchase" : "Achat potentiel"}</p><h2>{session.name}</h2></div><button type="button" className="danger" onClick={() => setConfirmDelete(true)}>{en ? "Delete session" : "Supprimer la session"}</button></div><div className="purchase-totals"><div><span>{en ? "Cards" : "Cartes"}</span><strong>{totals.count}</strong></div><div><span>{en ? "Cardmarket value" : "Valeur Cardmarket"}</span><strong>{formatPrice(totals.cardmarketTotal)}</strong></div><div><span>{en ? "Seller price" : "Prix vendeur"}</span><strong>{formatPrice(totals.sellerTotal)}</strong></div><div className={totals.difference !== null && totals.difference < 0 ? "is-saving" : ""}><span>{en ? "Saving" : "Économie"}</span><strong>{totals.difference === null ? "—" : formatDifference(-totals.difference)}</strong><small>{totals.differencePercent === null ? "" : `${totals.differencePercent > 0 ? "+" : ""}${totals.differencePercent.toLocaleString(language === "en" ? "en-GB" : "fr-FR", { maximumFractionDigits: 1 })} %`}</small></div></div><div className="purchase-session-controls"><label>{en ? "Sort" : "Trier"}<select value={sort} onChange={(event) => setSort(event.target.value as Sort)}><option value="deal">{en ? "Best deal %" : "Meilleure affaire %"}</option><option value="saving">{en ? "Largest saving €" : "Plus grosse économie €"}</option><option value="seller-asc">{en ? "Seller price low to high" : "Prix vendeur croissant"}</option><option value="seller-desc">{en ? "Seller price high to low" : "Prix vendeur décroissant"}</option><option value="market">{en ? "Cardmarket price" : "Prix Cardmarket"}</option><option value="added">{en ? "Order added" : "Ordre d’ajout"}</option></select></label></div><div className="purchase-items">{sortedItems.map((item) => <PurchaseItemRow key={item.id} item={item} selected={selectedItems.has(item.id)} onToggle={() => toggle(item.id)} onUpdatePrice={(sellerPrice) => purchases.updateSellerPrice(session.id, item.id, sellerPrice)} onUpdateFinish={(finish) => purchases.updateFinish(session.id, item.id, finish, impressions.find((candidate) => candidate.impressionId === item.impressionId))} onDelete={() => purchases.deleteItem(session.id, item.id)} />)}</div><div className="purchase-session-finish"><p>{selectedItems.size ? (en ? `${selectedItems.size} selected card${selectedItems.size > 1 ? "s" : ""} will be added to your collection only after confirmation.` : `${selectedItems.size} carte${selectedItems.size > 1 ? "s" : ""} sélectionnée${selectedItems.size > 1 ? "s" : ""} sera ajoutée à ta collection uniquement après confirmation.`) : (en ? "Select the cards you actually bought, then finish this session." : "Coche les cartes réellement achetées, puis termine la session.")}</p><button type="button" disabled={!selectedItems.size} onClick={() => setConfirmFinish(true)}>{en ? "Finish session" : "Terminer la session"}</button></div>{confirmFinish ? <div className="collection-confirm-backdrop" role="presentation"><section className="collection-confirm-dialog" role="dialog" aria-modal="true"><p className="eyebrow">{en ? "Finish session" : "Terminer la session"}</p><h2>{en ? `Add ${selectedItems.size} card${selectedItems.size > 1 ? "s" : ""} to your collection?` : `Ajouter ${selectedItems.size} carte${selectedItems.size > 1 ? "s" : ""} à ta collection ?`}</h2><p>{en ? "Only the checked cards will be marked as owned. The potential purchase session will remain available as your record." : "Seules les cartes cochées seront marquées comme possédées. La session d’achat potentiel restera disponible comme historique."}</p><div><button className="secondary" type="button" onClick={() => setConfirmFinish(false)}>{en ? "Cancel" : "Annuler"}</button><button type="button" onClick={() => { finishSession(); setConfirmFinish(false); }}>{en ? "Confirm and add" : "Confirmer et ajouter"}</button></div></section></div> : null}{confirmDelete ? <div className="collection-confirm-backdrop" role="presentation"><section className="collection-confirm-dialog" role="dialog" aria-modal="true"><p className="eyebrow">{en ? "Potential purchase" : "Achat potentiel"}</p><h2>{en ? "Delete this session?" : "Supprimer cette session ?"}</h2><p>{en ? "All cards saved in this potential purchase will be removed. Your collection will not change." : "Toutes les cartes de cet achat potentiel seront supprimées. Ta collection ne sera pas modifiée."}</p><div><button className="secondary" type="button" onClick={() => setConfirmDelete(false)}>{en ? "Cancel" : "Annuler"}</button><button className="danger" type="button" onClick={deleteSession}>{en ? "Delete" : "Supprimer"}</button></div></section></div> : null}</section>;
}

function PurchaseItemRow({ item, selected, onToggle, onUpdatePrice, onUpdateFinish, onDelete }: { item: PurchaseSessionItem; selected: boolean; onToggle: () => void; onUpdatePrice: (price: number | null) => void; onUpdateFinish: (finish: PurchaseFinish) => void; onDelete: () => void }) {
  const { language } = useSiteLanguage();
  const en = language === "en";
  ...
  return <article className="purchase-item-row"><label className="purchase-bought"><input type="checkbox" checked={selected} onChange={onToggle} /><span>{en ? "Bought" : "Achetée"}</span></label><CachedCardImage className="purchase-item-art" src={item.imageUrl ?? undefined} alt={item.name} /><div className="purchase-item-copy"><p className={`purchase-ownership ${item.collectionStatus === "owned" ? "is-owned" : "is-missing"}`}>{item.collectionStatus === "owned" ? (en ? `✓ Already owned · ×${item.ownedQuantity ?? 1}` : `✓ Déjà possédée · ×${item.ownedQuantity ?? 1}`) : (en ? "✕ Missing" : "✕ Manquante")}</p><h3>{item.name}</h3><p>{item.setName} · #{item.number} · {item.variant} · {item.rarity}</p><>{hasPurchaseFinish(item) ? <label className="purchase-finish-choice">{en ? "Version to buy" : "Version achetée"}<select value={item.finish ?? "normal"} onChange={(event) => onUpdateFinish(event.target.value as PurchaseFinish)}><option value="normal">{en ? "Normal (non-foil)" : "Normal (non foil)"}</option><option value="foil">Foil</option></select></label> : null}</><a className="collection-cardmarket-link" href={item.cardmarketUrl ?? "#"} target="_blank" rel="noopener noreferrer">{en ? "View on Cardmarket ↗" : "Voir sur Cardmarket ↗"}</a></div><dl><div><dt>Cardmarket{hasPurchaseFinish(item) ? ` · ${item.finish === "foil" ? "Foil" : "Normal"}` : ""}</dt><dd>{formatPrice(item.cardmarketPrice)}</dd><small>{item.priceMode === "low" ? (en ? "Lowest price" : "Prix minimum") : item.priceMode === "trend" ? (en ? "Trend" : "Tendance") : (en ? "30-day average" : "Moyenne 30 jours")}</small></div><label>{en ? "Seller price" : "Prix vendeur"}<input inputMode="decimal" value={priceInput} onChange={(event) => setPriceInput(event.target.value)} onBlur={commit} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} placeholder="—" /></label><div className={`purchase-difference is-${tone}`}><dt>{en ? "Difference" : "Écart"}</dt><dd>{formatDifference(difference)}</dd><small>{differencePercent === null ? "" : `${differencePercent > 0 ? "+" : ""}${differencePercent.toLocaleString(language === "en" ? "en-GB" : "fr-FR", { maximumFractionDigits: 1 })} %`}</small></div></dl><button type="button" className="purchase-remove" onClick={onDelete}>{en ? "Remove" : "Retirer"}</button></article>;
}
